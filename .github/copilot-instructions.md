...
- **Automation-First Remediation**: Prefer automated remediation first (`just fix <package>`, and `just lint-md` for Markdown issues). If automation is insufficient, explicitly report the required manual follow-up with file, issue, failure reason, and manual action.
